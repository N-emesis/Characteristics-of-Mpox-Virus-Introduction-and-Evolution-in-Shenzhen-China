...
nwk_path = "/mnt/d/hanjia/2.10/dnds/data/nextclade_shenzhen/nextclade.nwk"
output_dir = "/mnt/d/hanjia/2.10/tree_time_new"
...
new_to_orig = dict(zip(rename_df.iloc[:, 1], rename_df.iloc[:, 0]))
...
# Combine: new name -> lineage is already in new_to_lineage.
# For any name, tip_to_lineage will look it up directly.
tip_to_lineage = new_to_lineage.copy()
...
for term in tree.get_terminals():
    orig_name = new_to_orig.get(term.name, term.name)
    if orig_name in dates:
        anchor_depths.append(depths[term])
        anchor_dates.append(dates[orig_name])
...
def draw_clade(clade):
    ...
    if clade.clades:
        ...
        ax.plot([x, x], [y_min, y_max], color='black', lw=0.5)
        
        for child in clade.clades:
            cx = x_coords[child]
            cy = y_coords[child]
            ax.plot([x, cx], [cy, cy], color='black', lw=0.5)
            draw_clade(child)
...
for term in terminals:
    ...
    color = lineage_colors.get(lin, "#cccccc")
        
    ax.scatter(x, y, s=20, facecolor=color, edgecolor='black', linewidth=0.5, zorder=10)
...
time_span_days = max_date - min_date
if time_span_days > 365 * 10:
    locator = mdates.YearLocator(2)
    formatter = mdates.DateFormatter('%Y')
elif time_span_days > 365 * 3:
...
pdf_path = os.path.join(output_dir, "nextclade_tree_time_axis.pdf")
svg_path = os.path.join(output_dir, "nextclade_tree_time_axis.svg")
png_path = os.path.join(output_dir, "nextclade_tree_time_axis.png")

plt.savefig(pdf_path, bbox_inches='tight')
plt.savefig(svg_path, bbox_inches='tight')
plt.savefig(png_path, bbox_inches='tight', dpi=300)

print("Success! nextclade.nwk replotted using Nextclade lineages.")
