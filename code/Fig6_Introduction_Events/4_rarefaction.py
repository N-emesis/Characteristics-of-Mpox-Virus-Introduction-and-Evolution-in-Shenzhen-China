...
def make_regions_dict(wi_file, metadata_file):
	...
	usa_tips = {item['strain']: item['division'] for index, item in metadata[metadata['country'] == 'China'].iterrows()}
	global_tips = {item['strain']: item['country'] for index, item in metadata[metadata['country'] != 'China'].iterrows()}
	...
	for key, value in global_tips.items():
		if key not in included_keys:
			regions[key] = value
	# regions_df = pd.DataFrame(list(regions.items()), columns=['Sample', 'Region'])
...
def rarefaction(tre, aln_file, counties, state, all_regions_dict, n_samples, reps, out_path):
	...
	for n in n_samples:
		for rep in range(reps):
			...
			# unique_regions = [item for item in this_tre.get_terminals()]
			# for region in unique_regions:
			# 	print(region)
				
			# return 
			
			print(f"Number of available tips: {len(tips_list)}")
			print(f"Requested sample size: {n}")
			
			if n > len(tips_list):
				print(f"Warning: Requested sample size {n} is greater than available tips {len(tips_list)}. Using {len(tips_list)} instead.")
				n_actual = len(tips_list)
			else:
				n_actual = n
				
			sample_tips_list = random.sample(tips_list, k=n_actual)
...
def main():
	...
	parser.add_argument('--counties', help='name of counties to analyze', 
		nargs='+', default=('Shenzhen','SZ'))
	parser.add_argument('--state', 
		help='state to analyze, encompasses counties', default='Shenzhen')
	...
	parser.add_argument('--reps',
		help='number of replicates to do for each n', default=20)
	args = parser.parse_args()
	n_samples_input = [int(item) for item in args.n_samples]
	if len(n_samples_input) == 1:
		end = n_samples_input[0]
		n_samples = list(range(20, end, 20))
		if not n_samples or n_samples[-1] != end:
			n_samples.append(end)
	elif len(n_samples_input) == 2:
		start, end = n_samples_input
		step = 20
		if start > end:
			start, end = end, start
		n_samples = list(range(start, end + 1, step))
		if not n_samples or n_samples[-1] != end:
			n_samples.append(end)
	else:
		n_samples = n_samples_input
	print(f"Sampling sizes: {n_samples}")
