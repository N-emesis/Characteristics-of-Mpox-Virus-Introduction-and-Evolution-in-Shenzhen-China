...
def make_regions_dict(wi_file, metadata_file):
	...
	wi_tips= {item['Sample Name']: item['wisconsin_catchment_area'] for index, item in wi_regions.iterrows()}
...
def infer_timetree(tre, aln_file, dates, outgroup, resolve, out_path):
	# print(dates)
	# print("Date range:", min(dates.values()), max(dates.values()))
...
def infer_mugration(tre, aln_file, regions, out_path):
	tre, letter_to_state, state_to_letter = \
		wrappers.reconstruct_discrete_traits(tre, regions, sampling_bias_correction=2.5)
...
def main():
	...
	parser.add_argument('--tree_file', 
						...
						default='results/aligned.treefile')
